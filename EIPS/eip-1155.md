...
/**
    Note: The ERC-165 identifier for this interface is 0xa0fb7a6b.
*/
interface ERC1155ScopedApproval {
    ...
    */
    ...
    /**
        @dev MUST emit when the token ID is added to the scope. By default, IDs are in no scope.
    */
    event IdAddedToScope(uint256 indexed _id, bytes32 indexed _scope);

    /**
        @dev MUST emit when the token ID is removed from the scope.
    */
    event IdRemovedFromScope(uint256 indexed _id, bytes32 indexed _scope);

    /**
        @notice Queries the scope of a token ID.
        @param _id  The token ID
        @return     The scopes containing the ID
    */
    function getScopesForId(uint256 _id) external view returns (bytes32);
    ...
    */
    function isApprovedForScope(address _owner, address _operator, bytes32 _scope) external returns (bool);
}
```
